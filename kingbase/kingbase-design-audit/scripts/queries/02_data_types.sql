...
-- 适配 KingbaseES: 使用 format_type(a.atttypid, -1) 获得类型基名（避免 character varying(50) 不匹配 character varying），
-- 同时 a.atttypmod 仍用于检测是否有长度限制。
\pset border 0
...
WHERE a.attnum > 0 AND NOT a.attisdropped AND c.relkind IN ('r','p')
  ...
  AND format_type(a.atttypid, -1) <> 'boolean'
  AND (
    a.attname ~* '^(is_|has_|can_|should_|need_)' OR a.attname ~* '(_flag|_bool|_yn)$'
    OR col_description(c.oid, a.attnum) LIKE '%是否%'
  )
  AND format_type(a.atttypid, -1) IN ('integer','smallint','bigint','text','character varying')
ORDER BY 2,3,4;
...
WHERE a.attnum > 0 AND NOT a.attisdropped AND c.relkind IN ('r','p')
  ...
  AND format_type(a.atttypid, -1) IN ('text','character varying','character')
ORDER BY 2,3,4;
...
WHERE a.attnum > 0 AND NOT a.attisdropped AND c.relkind IN ('r','p')
  ...
  AND format_type(a.atttypid, -1) IN ('text','character varying')
ORDER BY 2,3,2;

-- [2.4] 主键/外键使用 varchar/text 而非 integer/bigint/uuid
...
WHERE con.contype IN ('p','f')
  AND n.nspname NOT IN ('pg_catalog','information_schema','pg_toast','sys_catalog','sys_hm','sysaudit','sysmac','src_restrict','anon','dbms_job','dbms_scheduler','kdb_schedule','perf','xlog_record_read','pg_bitmapindex')
  AND format_type(a.atttypid, -1) IN ('character varying','text')
ORDER BY 2,3,4;
...
WHERE a.attnum > 0 AND NOT a.attisdropped AND c.relkind IN ('r','p')
  AND n.nspname NOT IN ('pg_catalog','information_schema','pg_toast','sys_catalog','sys_hm','sysaudit','sysmac','src_restrict','anon','dbms_job','dbms_scheduler','kdb_schedule','perf','xlog_record_read','pg_bitmapindex')
  AND (
    (format_type(a.atttypid, -1) = 'character varying' AND a.atttypmod = -1)
    OR (format_type(a.atttypid, -1) = 'text' AND a.attname ~* '(_code|_no$|_id$|_status$|^code|^no_|^status)')
  )
ORDER BY 2,3,4;
...
WHERE a.attnum > 0 AND NOT a.attisdropped AND c.relkind IN ('r','p')
  AND n.nspname NOT IN ('pg_catalog','information_schema','pg_toast','sys_catalog','sys_hm','sysaudit','sysmac','src_restrict','anon','dbms_job','dbms_scheduler','kdb_schedule','perf','xlog_record_read','pg_bitmapindex')
  AND format_type(a.atttypid, -1) = 'text';
...
WHERE a.attnum > 0 AND NOT a.attisdropped AND c.relkind IN ('r','p')
  ...
  AND format_type(a.atttypid, -1) IN ('real','double precision')
ORDER BY 2,3,4;
...
WHERE a.attnum > 0 AND NOT a.attisdropped AND c.relkind IN ('r','p')
  ...
  AND format_type(a.atttypid, -1) NOT IN ('inet','cidr')
ORDER BY 2,3,4;
