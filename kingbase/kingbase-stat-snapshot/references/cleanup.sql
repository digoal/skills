-- ============================================================
-- kingbase-stat-snapshot / references / cleanup.sql
-- 历史数据清理：按时间保留 / 按快照数量保留，二选一或组合使用。
-- 与 PG 完全一致：金仓默认 PG 兼容模式，存储过程 / 动态 SQL / information_schema 行为一致。
-- ============================================================
...
-- ------------------------------------------------------------
-- 3. kdb_schedule 定时任务示例（金仓 R1C10 内置的任务调度扩展，等同 PG 的 pg_cron）
--    注意：必须在控制库（默认 kingbase 库）中创建，job 由 kingbase 后台执行。
-- ------------------------------------------------------------
-- 每天凌晨 3 点清理 7 天前的数据
-- CALL kdb_schedule.schedule('kingbase-stat-snapshot-cleanup-time', '0 3 * * *',
--     $$CALL stat_snapshot.cleanup_snapshots(7)$$);

-- 或者每天凌晨 3 点按数量保留最近 200 个快照
-- CALL kdb_schedule.schedule('kingbase-stat-snapshot-cleanup-count', '0 3 * * *',
--     $$CALL stat_snapshot.cleanup_by_count(200)$$);

-- ------------------------------------------------------------
-- 4. 操作系统 crontab 方式示例（无 kdb_schedule 时的替代方案）
--    需提前配置 .pgpass 或环境变量 PGPASSWORD 免密，避免在 crontab 中明文写密码。
-- ------------------------------------------------------------
-- 0 3 * * * PGPASSWORD="$(cat /etc/kingbase-stat-snapshot.pgpass)" psql "host=<HOST> port=<PORT> user=<USER> dbname=kingbase" -c "CALL stat_snapshot.cleanup_snapshots(7);" >> /var/log/kingbase-stat-snapshot-cleanup.log 2>&1