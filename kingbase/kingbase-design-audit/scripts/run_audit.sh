#!/usr/bin/env bash
# run_audit.sh :: kingbase-design-audit 编排脚本 (psql 版)
#
# 用法:
...
#   - 密码通过环境变量 PGPASSWORD 传入（KingbaseES 兼容 PG 客户端协议，沿用 PG 环境变量名），
#     不在命令行/脚本中硬编码或回显。
#   - 若不指定 -d，将自动扫描实例内所有非模板、允许连接的数据库。
#   - 每个数据库的每个检查项输出为独立文本文件，便于逐项审阅与后续汇总生成报告。
#
# 依赖: psql 客户端（KingbaseES 自带 ksql，但 psql 兼容此实例的 PG 协议）
...
# 1) 确定待扫描的数据库列表
# 先尝试 postgres (PG 习惯), 不存在则用本 skill 默认的 kingbase 数据库
if [[ -z "$DBS" ]]; then
  echo "未指定 -d，正在自动发现实例内所有非模板数据库..." >&2
  # 用一个肯定存在的库 (kingbase ES 默认内置) 拉 db 列表
  BOOT_DB=""
  for try_db in postgres kingbase; do
    if "${CONN_BASE[@]}" -d "$try_db" -c "SELECT 1" >/dev/null 2>&1; then
      BOOT_DB="$try_db"
      break
    fi
  done
  if [[ -z "$BOOT_DB" ]]; then
    echo "错误: 无法用 postgres/kingbase 任何一个数据库做引导查询连接实例" >&2
    exit 2
  fi
  DB_LIST=$("${CONN_BASE[@]}" -d "$BOOT_DB" -f "$QUERY_DIR/00_list_databases.sql")
else
  DB_LIST=$(echo "$DBS" | tr ',' '\n')
fi
...
for db in $DB_LIST; do
  ...
  # 跳过模板库
  if [[ "$db_trimmed" == "template0" || "$db_trimmed" == "template1" || "$db_trimmed" == "template2" ]]; then
    continue
  fi
