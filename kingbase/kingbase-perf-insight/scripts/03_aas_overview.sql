...
-- 注：KES 的 sys_stat_statements 无 wal_bytes 字段；本脚本额外输出金仓特有的解析/规划耗时
...
deltas AS (
  SELECT
    ...
    -- blk_read_time 依赖 track_io_timing=on，未开启时恒为0，需在报告中说明
    e.blk_read_time       - COALESCE(b.blk_read_time, 0)       AS delta_blk_read_time,
    -- 金仓特色：解析/规划耗时（对应 parses/plans）
...
SELECT
  ...
  -- 注：total_exec_time 列若为 double precision，SUM 结果为 double，需显式转 numeric 才能用 2 参 ROUND（PG/KES 均无 round(double,int)）
  ROUND((SUM(d.delta_total_exec_time) / NULLIF(EXTRACT(EPOCH FROM (w.end_time - w.begin_time)), 0) / 1000.0)::numeric, 2) AS aas,
  ...
  -- 金仓特色：解析+规划总耗时（毫秒）
  SUM(d.delta_total_parse_time + d.delta_total_plan_time) AS total_parse_plan_time_ms
...
-- 说明：KingbaseES 不直接暴露物理 vCPU 数，此处仅作参考；
