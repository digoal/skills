...
> 与 pg-design-audit 的差异：KingbaseES 在 PG 兼容模式下大部分系统视图 / 函数 / GUC 与 PG12 一致
> （当前测试实例 `server_version = 12.1`），因此 issue 标签和评分规则保持一致即可。
> 若使用金仓特有的 oracle / mysql 兼容模式，部分字段类型映射可能略有差异，请结合 `format_type()` 输出人工复核。
...
| `public_schema_open_create_review_needed` | 若 `public_role_can_create = true`，按 🔵 建议优化计 1 项 |
...
| 兼容模式 | 潜在影响 | 检查项应对方式 |
|---|---|---|
| PG 兼容模式（默认）| 与 PG 12 行为一致 | 本 skill 适用，无需调整 |
| Oracle 兼容模式 | 数值类型可能用 `NUMBER(p,s)`；时间类型可能用 `DATE` | `02_data_types.sql` 中 `format_type()` 返回值可能为 `numeric`、`timestamp without time zone` 等，金仓会自动映射到 PG 类型名；若映射结果不一致，需人工复核 |
| MySQL 兼容模式 | 字段类型可能含 `tinyint`、`mediumtext` 等 mysql 关键字眼 | 同上，金仓在 PG 兼容视图下应显示为 `smallint`、`text`，如仍出现 mysql 风格类型名则说明 schema 未走 PG 兼容模式 |
