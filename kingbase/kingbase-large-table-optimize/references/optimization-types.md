# 负载类型判定与优化模板（阶段 3）— KingbaseES 版

判定顺序：先检查是否同时满足两类以上条件 → 归为 E；否则按 A→B→C→D 顺序匹配第一个满足的类型。所有类型的前提都是「真实大小（修正膨胀后）> 10GB」，类型 D 额外要求 > 50GB。

语法基线说明：KingbaseES V9R1C10 兼容 PostgreSQL 12.1，以下 SQL 模板均为 PG12 及以下版本可用语法（已在实例验证：声明式分区、DETACH/ATTACH PARTITION、CREATE INDEX CONCURRENTLY、INCLUDE 覆盖索引、hash/brin 索引、物化视图均可用）。
...
2. **autovacuum 参数评估**：对比该表当前 DML 速率与默认 `autovacuum_vacuum_scale_factor`(0.2) / `autovacuum_vacuum_threshold`(50) 是否匹配（金仓默认值与 PG 一致），若死元组增长远快于默认阈值触发频率，建议对该表单独设置更激进参数：
   ```sql
...
3. **HOT 更新效率**：若 HOT 更新效率低（<50%），说明更新的列上有索引导致无法走 HOT 更新，建议：
   ...
   - 或调整 `fillfactor`（如设为 70~80）为行内更新预留空间，提升 HOT 命中率：
     ...
     ```
4. 若分区改造代价过高（大量外键依赖、应用层改造成本高），替代方案：**`sys_squeeze` 在线压缩**（金仓官方扩展，V9R1C10 可用，基于逻辑解码在线重组、无需长时间锁表，等价于 PG 生态的 pg_squeeze），而非 `VACUUM FULL`（会长时间持锁）：
   ```sql
   CREATE EXTENSION sys_squeeze;  -- 属 DDL，须用户授权
   SELECT sys_squeeze.squeeze_table(relid => 'schema.table'::regclass, ...);
   ```
...
4. **分区生命周期管理**：若分区数量多、手工维护成本高，评估 `kdb_partman`（金仓版 pg_partman，V9R1C10 可用）自动创建/归档分区（`CREATE EXTENSION kdb_partman;` 属 DDL，须用户授权）。
5. **写入热点**：最后一个分区往往是插入并发瓶颈，评估：
...
**核心问题**：数据量大导致 B-Tree 层级深（可用 `kbstatindex` 精确验证 `tree_level`），点查需要更多次随机 IO。

**优化建议模板**：

1. **索引瘦身**（针对标记「索引偏深」的索引，`tree_level > 3`）：
   ...
   - 覆盖索引（INCLUDE）：将查询常用的输出列放入 INCLUDE，避免回表：
     ...
     ```
   - Hash 索引：若查询全部是等值匹配（无范围查询），Hash 索引比 B-Tree 更紧凑（金仓支持 hash 访问方法，已验证）：
     ```sql
     CREATE INDEX CONCURRENTLY idx_table_hash ON schema.table USING hash (key_col);
     ```
2. **BRIN 索引**：若表的物理存储顺序与索引键强相关（如按插入时间自然排序的日志表），BRIN 体积远小于 B-Tree（金仓支持 brin 访问方法）：
   ```sql
   CREATE INDEX CONCURRENTLY idx_table_brin ON schema.table USING brin (created_at);
   ```
3. **应用层卸载**：评估连接池（pgbouncer 事务级连接池对金仓 PG 兼容模式同样适用）是否已启用（减少连接开销）+ 应用层缓存热点点查结果，降低对数据库的直接点查压力。所有 `CREATE INDEX` 建议使用 `CONCURRENTLY` 避免锁表，但仍需业务低峰期执行并监控进度。
...
1. **列存储评估**：若该表主要用于报表/聚合分析（非点查），评估金仓的 `cstore_fdw` 列存方案（金仓官方文档「插件参考」收录；需目标实例已安装该扩展，未安装则先确认可行性，`CREATE EXTENSION cstore_fdw;` 属 DDL，须用户授权）。可显著降低扫描 IO；需评估迁移成本与是否有实时写入需求（列存对高频写入不友好）。
2. **物化视图**：对固定的聚合查询模式（如按天/按维度汇总），创建物化视图并定时刷新（金仓支持物化视图；定时调度可用金仓的 `kdb_schedule` 扩展替代 PG 生态的 pg_cron）：
   ```sql
   ...
   ```
3. **只读副本卸载**：将分析类查询路由到只读备库（金仓支持物理流复制只读备机），避免与主库 OLTP 负载争抢 CPU/IO；需确认已有只读副本，若无则需先评估新增副本的成本。
