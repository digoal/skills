#!/usr/bin/env bash
# kingbase-parameter-tuning-advisor: 操作系统 / 存储 / 网络侧只读采集脚本
#
# 用途：在已获得用户授权的前提下（同机执行，或通过 SSH 登录到金仓所在主机），
...
if [ -n "$PG_DATA_DIR" ]; then
  ...
  df -h "$PG_DATA_DIR" 2>/dev/null || echo "无法定位该路径，请确认路径是否可读"
...
section "正在运行的 kingbase 进程（确认是否同机）"
if has_cmd pgrep; then
  pgrep -a kingbase 2>/dev/null | head -10 || echo "未发现 kingbase 进程（可能不在本机）"
else
  ps -ef | grep '[k]ingbase' | head -10 || echo "未发现 kingbase 进程（可能不在本机）"
fi
