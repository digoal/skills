-- ============================================================
-- kingbase-stat-snapshot / references / ddl_perdb.sql
-- 库级历史表：需要连接到每一个非模板数据库分别执行一次（IF NOT EXISTS，可重复执行）。
-- 遍历库列表：SELECT datname FROM pg_database WHERE datistemplate = false;
--
-- 与 PG 兼容模式一致：金仓默认存在 pg_stat_user_tables / pg_stat_user_indexes /
-- pg_statio_user_tables / pg_statio_user_indexes（pg_catalog 与 sys_catalog 均提供）。
-- ============================================================

CREATE SCHEMA IF NOT EXISTS stat_snapshot;

-- ------------------------------------------------------------
-- 0. 快照元数据表（每库一份，INSERT 元数据行时使用本库的 connection）
-- ------------------------------------------------------------
