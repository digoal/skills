# KingbaseES 关键参数调优知识库

本文件是 `kingbase-parameter-tuning-advisor` skill 的参考资料，供 Step 5（生成参数调整建议）查阅经验公式和取值范围。**所有公式都是起点，不是终点**——最终建议值必须结合 Step 2/3/4 实际采集到的证据做修正，不能直接套用。

> 说明：KingbaseES 默认 PG 兼容模式，以下参数名、单位、`reload`/`restart` 分类与 PostgreSQL 一致（均在 V9R1C10 实例上验证存在）。配置文件名是 `kingbase.conf`，会话级 reload 用 `SELECT sys_reload_conf();`（与 `pg_reload_conf()` 等价），服务器侧命令为 `sys_ctl reload/restart -D <data_dir>`。参数当前值一律用 `SHOW <name>` 或 `pg_settings`/`sys_settings` 读取。

## 一、内存类参数

### shared_buffers
- 作用：KingbaseES 自己管理的共享缓冲区，缓存表/索引页面。
...
### max_connections
- 作用：允许的最大客户端连接数。
- 判断依据：`pg_stat_activity` 里的实际并发连接数、以及 `idle in transaction`/`idle` 状态占比。若空闲连接占比很高，优先建议应用层引入连接池（金仓场景同样适用 PgBouncer 等），而不是单纯加大 `max_connections`——连接数过多会增加上下文切换开销，且和 `work_mem` 乘积关系直接影响内存上限。
...
### wal_buffers
- 作用：WAL 写入前的缓冲区。
- 经验起点：`shared_buffers` 的 **1/32** 左右，常见设为 16MB（金仓与 PG 一样在 -1 时自动按此公式估算，多数场景无需手工设置过大）。
- 生效方式：**restart**。
...
- 经验起点：**0.9**（默认已是较好值，一般不需要大改；注意 V9R1C10 默认值是 0.5，若检查点造成 IO 尖峰可考虑提到 0.9 附近）。
- 生效方式：**reload**。

### checkpoint_timeout
- 作用：定时 checkpoint 的最大间隔。
- 经验起点：**15min~30min**（写入密集场景可适当延长配合更大的 `max_wal_size`；V9R1C10 默认 5min，写密集时可加大）。
- 生效方式：**reload**。
...
- 判断依据：复杂查询（多表 join、范围查询）执行计划频繁偏离预期、`sys_stat_statements` 中某些查询的估算行数和实际行数差距很大。
...
- 判断依据：对写入延迟极度敏感、可以接受极小概率丢失最近事务的场景，可考虑 `off` 或 `local`；有强一致性要求（金融类）应保持 `on` 甚至配合同步复制（金仓 RWC 读写分离集群场景同理）。
- 生效方式：**reload**，且可以按会话级别 `SET` 覆盖，无需全局强改。

## 八、KingbaseES 特有的采集/诊断项（非参数，但影响判断）

| 项目 | 说明 | 使用方法 |
|------|------|----------|
| database_mode | 当前库兼容模式（pg/oracle/mysql），影响部分 SQL 行为 | `SHOW database_mode;` |
| sys_stat_statements | 金仓版的 pg_stat_statements（public schema），列结构与 PG14+ 一致，`*_exec_time` 单位为毫秒 | 需 `shared_preload_libraries` 含 `sys_stat_statements`；`SELECT ... FROM sys_stat_statements` |
| sys_catalog.sys_stat_* | 金仓原生动态性能视图族（sys_stat_database / sys_stat_bgwriter / sys_stat_activity / sys_stat_sql / sys_stat_sqltime / sys_stat_wait / sys_stat_dmlcount 等），带 Oracle 风格的等待事件/TOP SQL 语义 | 需显式 `sys_catalog.` 前缀访问 |
| sys_kwr / KWR | 金仓 AWR 式性能报告（sys_kwr 扩展），可辅助定位历史负载 | `SHOW shared_preload_libraries` 检查是否加载；报告生成参考官方文档 |
| kbbadger | 金仓日志分析工具（类似 pgBadger），从日志提取慢查询/错误/checkpoint 频率 | 官方文档：性能调优指南-性能调优工具 |
| 内置 schema | sys_catalog / sys_hm / sysaudit / sysmac / src_restrict / dbms_job / dbms_scheduler 等为金仓自身维护对象 | 分析 workload 时过滤，避免误判 |

## 九、操作系统层面配套参数（非 kingbase.conf，但直接影响数据库表现）
