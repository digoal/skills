...
KingbaseES 与 PostgreSQL 语法高度兼容，以下 DDL 均可在金仓直接执行；涉及扩展时先确认金仓已安装对应扩展（如 `pg_trgm` 在金仓 V9 中通常已内置，可用 `CREATE EXTENSION IF NOT EXISTS pg_trgm;` 按需启用）。

## 1. 缺少合适的索引

判断信号：
- 需要执行计划辅助确认（如用户授权，可对该 SQL 跑 `EXPLAIN (ANALYZE, BUFFERS)`，金仓语法与 PG 一致）。
...
信号：出现在「执行频率 TOP」或「写放大 TOP」（金仓无 wal_bytes，以脏块数判断写压力），且为单行 INSERT/UPDATE/DELETE 高频执行。
...
- 金仓兼容层参数（如 `database_mode`、`ora_input_emptystr_isnull` 等）改动影响面大，不要作为单条 SQL 优化手段提出。
