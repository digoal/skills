...
WHERE indrelid IN (
  ...
  WHERE n.nspname NOT IN ('pg_catalog','information_schema','pg_toast','sys_catalog','sys_hm','sysaudit','sysmac','src_restrict','anon','dbms_job','dbms_scheduler','kdb_schedule','perf','xlog_record_read','pg_bitmapindex')
)
...
-- 注意：KingbaseES 目录不记录索引创建时间，"超过7天"需结合 pg_stat_database.stats_reset 人工判断，
...
WHERE s.idx_scan = 0
  ...
  AND s.schemaname NOT IN ('pg_catalog','information_schema','pg_toast','sys_catalog','sys_hm','sysaudit','sysmac','src_restrict','anon','dbms_job','dbms_scheduler','kdb_schedule','perf','xlog_record_read','pg_bitmapindex')
ORDER BY 2,3;
