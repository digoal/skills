...
-- 注：KES 的 pg_stat_database 视图与 PG12 一致，直接使用
...
-- 方式B：从 sys_stat_statements 差值按 dbid 聚合总耗时占比（与快照关联需 pg_database 快照或 oid->name 映射，
...
SELECT
  ...
  ROUND((SUM(delta_total_exec_time) / NULLIF(SUM(SUM(delta_total_exec_time)) OVER (), 0) * 100)::numeric, 1) AS pct_of_total
FROM deltas
GROUP BY dbid
