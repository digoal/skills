#!/usr/bin/env bash
# kingbase-find-unused-index 批量扫描脚本
#
# 用法:
...
#
# 依赖: psql（KingbaseES 自带 ksql / ksql 客户端也可）
...
# KingbaseES 默认并不像 PG 那样创建名为 postgres 的库，因此先用当前 dbname
# 连一下并通过 pg_database 取一个可连接的 admin db（默认即 PGDATABASE）。
ADMIN_DB="${PGDATABASE:-kingbase}"
ADMIN_CONN="host=${HOST} port=${PORT} user=${PGUSER} dbname=${ADMIN_DB} sslmode=prefer"

echo "==> 正在列出实例下所有可连接数据库 (admin_db=${ADMIN_DB})..."
# 使用 while-read 替代 bash 4+ 的 mapfile，保持对 bash 3.2 (macOS) 兼容
DATABASES=()
