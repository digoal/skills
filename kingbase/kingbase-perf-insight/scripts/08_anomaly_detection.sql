...
SELECT
  ...
  ROUND((b.total_exec_time / NULLIF(b.calls, 0))::numeric, 2) AS avg_latency_begin_ms,
  ROUND((e.total_exec_time / NULLIF(e.calls, 0))::numeric, 2) AS avg_latency_end_ms,
  ROUND(
    ((e.total_exec_time / NULLIF(e.calls, 0)) / NULLIF(b.total_exec_time / NULLIF(b.calls, 0), 0))::numeric,
    2
  ) AS latency_ratio
FROM {schema}.sys_stat_statements_snapshot e
JOIN {schema}.sys_stat_statements_snapshot b
...
WHERE e.snapshot_id = :snap_end_id
  AND NOT EXISTS (
    SELECT 1 FROM {schema}.sys_stat_statements_snapshot b
