...
1. 先用 `references/manual_checks.sql` 中的 A5 确认 `pg_stat_statements` 是否已安装
   （用于后续更系统地定位劣化查询，非本次验证的硬性前提）。
...
| `pg_stat_statements` 未安装导致无法追溯历史慢查询 | 不强制要求安装；提示用户如需更系统地定位劣化 SQL 可自行安装该扩展，本次仅基于用户提供的具体查询做验证 |
