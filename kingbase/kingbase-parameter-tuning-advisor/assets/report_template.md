...
## 实例概览
- 版本：{{server_version}}
- 兼容模式：{{database_mode}}（pg / oracle / mysql）
- 配置文件：{{config_file}}（一般为 kingbase.conf）
...
> 生效方式说明：`reload` = 执行 `SELECT sys_reload_conf();`（或 `sys_ctl reload -D <data_dir>`）；`restart` = 需要重启实例（`sys_ctl restart`）才能生效。
...
---
本报告仅提供参数调整建议，不包含可直接执行的变更脚本；请在测试环境验证后再应用到生产，并遵循贵司变更管理流程。金仓相关官方文档见 https://docs.kingbase.com.cn/cn/KES-V9R1C10/category/性能调优指南
