-- ============================================================
-- kingbase-stat-snapshot / references / delta_templates.sql
...
--
-- 与 PG 的差异：所有 sys_stat_statements 字段名、字段顺序与 PG 一致，
-- 可直接复用 pg_stat_statements 的差值计算方式。
-- ============================================================

-- ------------------------------------------------------------
-- 1. TOP SQL 分析（sys_stat_statements 差值，按耗时排序）
-- ------------------------------------------------------------
SELECT
    e.queryid,
    e.query,
...
--    KES R1C10 (server_version_num=12xxxx) 不存在该视图，此模板为未来版本预留。
-- ------------------------------------------------------------
SELECT
...
-- ------------------------------------------------------------
-- 6. 直接调用通用差值函数（仅覆盖 sys_stat_statements 场景，见 ddl_core.sql）
-- ------------------------------------------------------------
