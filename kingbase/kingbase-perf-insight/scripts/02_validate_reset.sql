...
-- （或用 psql -v snap_begin_id=... -v snap_end_id=... 传参，或 python 脚本 --snap-begin/--snap-end）
...
-- 建议将分析窗口调整为该时间点之前或之后。
-- 注：KES 无 sys_stat_statements_info 视图，source_reset_time 通常由采集端记录
-- （如采集时的实例启动时间 pg_postmaster_start_time() 或数据库 stats_reset）。
