-- ============================================================
-- kingbase-stat-snapshot / references / ddl_optional.sql
...
--
-- 与 PG 的关键差异：
--   1) KES R1C10 (server_version_num=12xxxx) 没有 pg_stat_wal / sys_stat_wal（PG14+）
--   2) KES R1C10 没有 pg_stat_checkpointer / sys_stat_checkpointer（PG17+）
--   3) sys_stat_replication 与 pg_stat_replication 同时存在，前者在 sys_catalog
-- ============================================================

-- ------------------------------------------------------------
-- 1. pg_stat_wal（PG 14+ 才有，金仓 R1C10 暂不支持；保留模板便于未来版本直接启用）
-- ------------------------------------------------------------
DO $$
BEGIN
...
--    金仓同时提供 sys_catalog.sys_stat_replication 同义视图
-- ------------------------------------------------------------
DO $$
BEGIN
    IF NOT EXISTS (
...
--    KES R1C10 只有 pg_stat_bgwriter，没有 pg_stat_checkpointer。
-- ------------------------------------------------------------
DO $$
BEGIN
    IF NOT EXISTS (
