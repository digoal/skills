#!/usr/bin/env bash
# kingbase-runtime-risk / run_scan.sh
#
# KingbaseES（金仓）只读运行时风险扫描脚本（psql 版）。
# 所有查询均在只读事务中执行，不做任何写操作。
#
# 依赖：psql（≥10，金仓自带 ksql / sys_ksql 或任意 PG 客户端均可）
...
#
# 用法：
#   PGPASSWORD='xxx' ./run_scan.sh [-h <host>] [-p <port>] [-U <user>] [-d <db>] [-o <outdir>]
#   # 全部省略时自动使用环境变量/默认值，如：
#   PGPASSWORD='xxx' ./run_scan.sh
#
# 输出：
#   在 <output_dir>（默认 ./kingbase_runtime_risk_output_<timestamp>）下生成一系列
#   .csv 文件，每个文件对应报告中的一个检查项。Agent 读取这些文件后
...
usage() {
  ...
  echo "      连接参数缺省时依次回退到环境变量与默认值(127.0.0.1/5432/kingbase/kingbase/123456)" >&2
  exit 1
}
...
USER="${USER:-${PGUSER:-kingbase}}"
# PGDBNAME 与 PGDATABASE 等价（金仓手册常用 PGDBNAME，本技能统一支持两者）
DB="${DB:-${PGDBNAME:-${PGDATABASE:-kingbase}}}"
if [[ -z "${PGPASSWORD:-}" ]]; then
  # 未提供密码时回退到默认值（仅适用于本地开发实例，生产环境必须显式提供）
  export PGPASSWORD="123456"
  echo "[提示] 未设置 PGPASSWORD 环境变量，使用默认密码（仅适用于本地开发实例）" >&2
fi

if [[ -z "$OUTDIR" ]]; then
  OUTDIR="./kingbase_runtime_risk_output_$(date +%Y%m%d_%H%M%S)"
fi
mkdir -p "$OUTDIR"
...
# 注意：必须使用 SET TRANSACTION READ ONLY（作用于当前事务），而不是
# SET SESSION CHARACTERISTICS（只影响后续事务，当前事务仍可写）；
...
echo "==> 连接: host=$HOST port=$PORT user=$USER db=$DB"
echo "==> 输出目录: $OUTDIR"

# 金仓内置 schema，扫描用户业务对象时统一过滤，避免把内部表/序列误报为业务风险
KBUILTIN_SCHEMAS="'sys_catalog','pg_catalog','information_schema','sys_hm','sysmac','sysaudit','src_restrict','anon','perf','xlog_record_read','dbms_job','dbms_scheduler','kdb_schedule','pg_bitmapindex'"
...
WHERE c.relkind IN ('r','m','p')
  AND n.nspname NOT IN ($KBUILTIN_SCHEMAS)
...
# 使用 \gset + \gexec/\if 组合：先生成包含元数据与分级判断的联合查询文本，
# 再执行该文本，全程只读（仅 SELECT，不调用 nextval/setval）。
{
  # 使用带引号的 heredoc（不做 shell 变量/反引号展开，避免 SQL 内容被 shell 劫持），
  # 内置 schema 列表通过 __KBUILTIN_SCHEMAS__ 占位符 + sed 注入。
  cat <<'EOSQL'
BEGIN;
...
SELECT string_agg(
  format(
    'SELECT %L::text AS seq_full_name, %L::boolean AS is_cycle, last_value,
            CASE WHEN %L THEN NULL WHEN %L THEN NULL
                 ELSE floor((%s::numeric - last_value) / NULLIF(%s,0)) END AS remaining_calls,
            CASE
              WHEN %L THEN ''循环序列(不回卷但可能重复)''
              WHEN %L THEN ''降序序列(需人工评估)''
              WHEN last_value IS NULL THEN ''未知(从未调用)''
    ...
    s.seqcycle, s.seqincrement < 0, s.seqmax, s.seqincrement,
    s.seqcycle, s.seqincrement < 0,
...
JOIN pg_namespace n ON c.relnamespace = n.oid
WHERE n.nspname NOT IN (__KBUILTIN_SCHEMAS__)
\gset

\if :{?combined_query}
:combined_query;
\else
SELECT '当前实例(用户 schema)没有序列对象' AS note;
\endif

COMMIT;
EOSQL
} | sed "s|__KBUILTIN_SCHEMAS__|$KBUILTIN_SCHEMAS|g" | "${PSQL_BASE[@]}" --csv > "$OUTDIR/02_sequence_risk.csv" 2> "$OUTDIR/02_sequence_risk.csv.err"
...
# pg_ls_waldir() 需要超级用户或 sys_monitor 角色权限，权限不足时优雅降级。
...
# 金仓内置对象（sys_catalog/sysmac/sys_hm 等）以及 sys_stat_statements 统计表
# 自带 oid 列，需过滤，避免把系统内部对象误报为"大对象引用列"。
...
WHERE (a.atttypid = 'oid'::regtype OR a.atttypid = to_regtype('lo'))
  ...
  AND n.nspname NOT IN ($KBUILTIN_SCHEMAS, 'pg_toast')
...
# ---------- 第九部分：统计信息过时预警 ----------
# 采用 pg_stat_user_tables.n_mod_since_analyze（autovacuum 判断是否触发 autoanalyze
# 的依据本身），比人工用累计 n_tup_ins/upd/del 估算更准确，且天然在每次 ANALYZE 后
# 归零，不需要额外基准点。同时读取每张表的 reloptions，识别表级覆盖的
