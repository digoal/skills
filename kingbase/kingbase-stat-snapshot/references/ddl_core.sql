-- ============================================================
-- kingbase-stat-snapshot / references / ddl_core.sql
-- 核心基础设施：快照元数据表 + 实例级历史表
--   - sys_stat_statements (位于 public schema, 默认已安装为扩展)
--   - pg_stat_activity     (金仓默认PG兼容视图)
--   - pg_stat_replication  (主库)
--   - pg_stat_database     (实例级汇总)
--   - pg_stat_bgwriter     (后台写入/检查点)
--
-- 在一个固定的"控制库"中执行一次即可（默认用 current_database() 即 KINGBASE 库）。
-- 全部使用 IF NOT EXISTS，可重复执行。
--
-- 与 PG 的关键差异：
--   1) sys_stat_statements 默认装在 public schema；通过 CREATE EXTENSION sys_stat_statements 安装
--   2) 没有 sys_stat_statements_info 视图；改用 sys_stat_statements_get_reset_time() 函数
--   3) KES R1C10 (server_version_num=12xxxx) 没有 pg_stat_wal / sys_stat_wal (PG14+)
--   4) KES R1C10 没有 pg_stat_checkpointer / sys_stat_checkpointer (PG17+)
--   5) sys_stat_statements_all 视图额外提供 parent_queryid 字段
-- ============================================================
...
-- ------------------------------------------------------------
-- 2. sys_stat_statements 历史表（实例级，最重要，动态建表适配版本差异）
--    sys_stat_statements 安装后位于 public schema；为兼容 PG 习惯，本 skill 同样从 public 读取。
--    query 字段统一用 text 避免被 track_activity_query_size 二次截断。
-- ------------------------------------------------------------
DO $$
BEGIN
    IF NOT EXISTS (
...
--    金仓 PG 兼容模式默认存在 pg_catalog.pg_stat_activity
-- ------------------------------------------------------------
DO $$
BEGIN
    IF NOT EXISTS (
...
-- ------------------------------------------------------------
-- 示例：检测 public.sys_stat_statements 相对历史表新增的字段
-- SELECT a.attname
-- FROM pg_attribute a
...
-- ------------------------------------------------------------
-- 5. 通用差值计算函数（sys_stat_statements 两个快照差值）
--    自动校验 reset 一致性并过滤负增长条目
-- ------------------------------------------------------------
...
COMMENT ON FUNCTION stat_snapshot.compute_delta(bigint, bigint) IS
    'sys_stat_statements 两个快照之间的差值计算，自动校验 reset 一致性并过滤负增长条目';