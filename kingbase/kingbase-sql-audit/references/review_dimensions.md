# kingbase-sql-audit 逐条审查项详解

本文件是 SKILL.md 的详细展开版本，供审查过程中按需查阅。每个维度都给出：检查点、判断标准、典型修正写法。除特别标注外，所有 SQL 均与 PostgreSQL 兼容（金仓 `database_mode=pg`）。
...
| `WHERE col LIKE '%abc'` | 前缀通配符无法用 btree | ⚠️ 金仓差异：V9R1C10 默认**不提供 pg_trgm 扩展**（`pg_available_extensions` 实测无）。可选方案：① 改用全文检索（`to_tsvector` + GIN 索引）匹配业务语义；② 与业务沟通改为前缀匹配 `LIKE 'abc%'`（可用 btree）；③ 数据量小或低频则接受 Seq Scan 并在报告中说明 |

**索引建议分级**
- 高频 SQL（OLTP、每秒/每分钟级调用）：必须给出具体 `CREATE INDEX CONCURRENTLY` 建议（金仓实测支持，须在事务块外单独执行）
...
- `ALTER TABLE ... ADD COLUMN ... DEFAULT <非常量表达式>`（常量默认值不重写，函数/易变默认值仍会重写）
...
**并发友好写法（金仓实测支持）**
- 建索引：`CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_xxx ON tbl (col);`（**不可在事务块内执行**，需单独会话/单独 psql -c）
- 加约束：先 `ADD CONSTRAINT ... NOT VALID`，再择机 `VALIDATE CONSTRAINT`（VALIDATE 只加 SHARE UPDATE EXCLUSIVE 锁，不阻塞读写；金仓实测支持）
...
- 警惕忘记 `COMMIT`/`ROLLBACK` 导致 `idle in transaction`，建议设置 `idle_in_transaction_session_timeout`（金仓实测支持该参数）
...
| `CREATE INDEX` | `DROP INDEX CONCURRENTLY` | 风险低（金仓实测支持） |
...
**函数/存储过程中动态 SQL 检查点**（金仓 plpgsql 与 PG 兼容）
...
- 建议：大操作放在低峰期，分批执行，必要时监控 `pg_stat_replication` 的 `replay_lag`（金仓实测视图存在）

**资源消耗评估**
- 结合 `work_mem`、`shared_buffers` 判断大排序/哈希 Join 是否会溢出到磁盘（对照执行计划中的 `Sort Method` / `Hash Batches`）
- 金仓环境负载画像：若实例启用了 `sys_stat_statements`（金仓版 pg_stat_statements，位于 `public` schema），可查询 `total_exec_time/calls/rows` 评估目标表相关 SQL 的累计资源消耗；未启用则记为"环境负载画像不可用"
